...
main(void)
{
  ...
  for (i = 0; i <= 100; ++i)
    {
      double xl= (1 - i / 100.0) * x[0] + (i / 100.0) * x[N-1];
      double yi_cubic = gsl_spline_eval(spline_cubic, xl, acc);
      double yi_akima = gsl_spline_eval(spline_akima, xl, acc);
      double yi_steffen = gsl_spline_eval(spline_steffen, xl, acc);

      printf("%g %g %g %g %d\n", xl, yi_cubic, yi_akima, yi_steffen,i);
    }
